...
Crawling starts from a seed URL and follows links, scraping every page it finds. It runs as a job on the server. For a single page use [`scrape()`](../README.md#usage) instead.
...
Crawl methods (`crawl`, `crawlAsync`, `getJob`, `cancelJob`) throw `WebCrawlerAPI\Exceptions\WebcrawlerApiException` for API and network failures:
...
try {
    ...
    $e->getErrorCode();    // e.g. "network_error", "timeout", or the API's error_code
    $e->getErrorMessage(); // human-readable message
    $e->getStatusCode();   // HTTP status, 0 for network errors
}
```

Error codes are defined in `WebCrawlerAPI\ErrorCode`. The underlying Guzzle exception is available via `$e->getPrevious()`.
